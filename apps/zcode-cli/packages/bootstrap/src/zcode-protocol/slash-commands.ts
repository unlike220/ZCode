...
export interface ListProtocolSlashCommandsOptions extends ListZCodeCustomCommandsOptions {
  /**
   * 动态工作流开关。只有显式 true 才保留内置 `workflow`。
   * 缺席按关闭处理；协议服务端从 appRuntimePreferences 传入显式布尔。
   */
...
export async function listProtocolSlashCommands(
  ...
  const dynamicWorkflowEnabled = resolveDynamicWorkflowEnabled(options.dynamicWorkflowEnabled);
  const builtins = listAppProtocolBuiltinSlashCommands().filter(
    (command) => dynamicWorkflowEnabled || command.name !== BUILTIN_WORKFLOW_COMMAND_NAME,
  );
